...
A connected feature spanning the building catalogue, placement controls, world interactions, processing-station queues and storage containers. Weapon racks and incubators illustrate a shared item-selection interface with different gameplay rules. The challenge is to turn changing gameplay rules into clear player actions while keeping UI state, world-object identity and native commands aligned.
...
[中文案例](docs/README.zh-CN.md) · [Full case study](docs/CASE_STUDY.md) · [Shared interactions](docs/SHARED_INTERACTIONS.md) · [Architecture](docs/ARCHITECTURE.md) · [Code tour](docs/CODE_TOUR.md) · [Refresh and performance](docs/PERFORMANCE.md) · [Debugging and edge cases](docs/DEBUGGING.md)
...
flowchart TB
    ...
    D --> I[Weapon rack: choose an item to display]
    D --> J[Incubator: choose a compatible egg]
    G --> H[Return to the game world]
    F --> H
```

These are alternative activities available after building, not a mandatory sequence or one universal state machine. The processing example is the **semi-finished-goods production panel**, not the separate equipment-upgrade workbench. Incubator time is queried through interaction notices, not the processing panel's progress UI.
...
**4. Reuse at the right boundary.** Weapon racks and incubators share an item-selection frame, title, rows and lifecycle rules. Their adapters retain different eligibility and command semantics: a rack uses the item's source container/position; an incubator checks compatible eggs and queries a free slot when clicked. [Shared interaction case](docs/SHARED_INTERACTIONS.md). Separately, storage retains slot data at stable capacity and refreshes displayed entries. [Implementation and cost boundaries](docs/PERFORMANCE.md).

## Gameplay media

**Screenshots and clips have not yet been added.** The [media section](media/README.md) defines captures and English captions for placement, processing, storage, weapon-rack selection and incubator selection/notices. Diagrams describe relationships; they are not substitute screenshots of the shipped game.
...
5. [Shared interaction frame](Content/Lua/GameLogics/Interaction/Addition/AdditionFrame.lua) → [weapon-rack adapter](Content/Lua/GameLogics/Interaction/Addition/SubPartModel/AdditionWeaponShelfModel.lua) / [incubator adapter](Content/Lua/GameLogics/Interaction/Addition/SubPartModel/AdditionEggModel.lua): one selection surface, different commands.
6. [Tests](docs/TESTING.md): focused Lua checks and clearly separated runtime-validation targets.
